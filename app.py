...
st.info("📌 Upload a brain MRI image. The system will first detect whether a tumor exists, and if present, identify its type.")
...
if uploaded_file is not None:
    ...
    if st.button("🔍 Detect Tumor"):
        ...
        if tumor_type == "notumor":
            ...
            st.write(f"🧬 **Tumor Type:** `{tumor_type.capitalize()}`")
            st.write(f"📊 **Confidence:** `{confidence:.2f}%`")

            st.warning(
                "⚕️ This prediction is generated by an AI model and should be verified by a medical professional."
            )

        # Probability distribution (NEW FEATURE ⭐)
...
# Footer
st.markdown("---")
st.caption("🚀 Academic Project | Computer Vision | Brain Tumor Detection using CNN (ResNet50)")
